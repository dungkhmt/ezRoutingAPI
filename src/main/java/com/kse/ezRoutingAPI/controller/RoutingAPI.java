	...
	public SolutionRoutes computePickupDeliveryRoutePlan(@RequestBody PickupDeliveryProblemInput input){
		...
		*/
		...
		for(Point p: requestPoints){
			...
			System.out.println(name() + ":: computeRoute finished, point " + p.ID + "\t accDemand = " + accDemand.get(p).getValue() +
					"\t eat = " + eat.getEarliestArrivalTime(p));
		}
		for(int i = 0; i < input.getVehicles().length; i++){
			System.out.println("Vehicle.Capacity = " + input.getVehicles()[i].getCapacity());
		}
		System.out.println("XR = " + XR.toString() + ", F = " + F.getValues().toString());
