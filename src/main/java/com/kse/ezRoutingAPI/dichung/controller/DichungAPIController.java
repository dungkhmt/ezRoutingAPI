...
public class DichungAPIController {
	...
	public void writeRequest(SharedTaxiInput input, SharedTaxiSolution sol) {
		try {
			...
			// String fn = ROOT_DIR + "dichungairport-requests-" + s[0] + s[1] +
			// s[2] + "-" + s[3] + s[4] + s[5] + ".txt";
			String fn = dir + "/dichungairport-requests-" + s[0] + s[1] + s[2]
					+ "-" + s[3] + s[4] + s[5] + ".txt";
			System.out.println(name() + "::writeRequest to file " + fn);
			PrintWriter out = new PrintWriter(fn);
			out.println("airport: (address \t lalng)");
			out.println(input.getAirportAddress() + "\t"
					+ input.getAirportPos());
			...
			for (int i = 0; i < input.getRequests().length; i++) {
				...
				out.println(r.getChungName() + "\t" + r.getPickupAddress()
						+ "\t" + r.getPickupPos() + "\t"
						+ r.getDeliveryAddress() + "\t" + r.getDeliveryPos()
						+ "\t" + r.getDepartTime() + "\t"
						+ r.getNumberPassengers());
			}
	...
	public void writeGlobalRequest(GlobalSharedTaxiInput input) {
		try {
			...
			// String fn = ROOT_DIR + "dichungairport-requests-" + s[0] + s[1] +
			// s[2] + "-" + s[3] + s[4] + s[5] + ".txt";
			String fn = dir + "/dichung-global-requests-" + s[0] + s[1] + s[2]
					+ "-" + s[3] + s[4] + s[5] + ".txt";
			System.out.println(name() + "::writeRequest to file " + fn);
			PrintWriter out = new PrintWriter(fn);
	...
	public SharedTaxiSolution[] computeSharedTaxiSolution(
			HttpServletRequest request, @RequestBody GlobalSharedTaxiInput input) {
		String path = request.getServletContext().getRealPath(
				"ezRoutingAPIROOT");
		System.out.println(name() + "::computeSharedTaxiSolution, path = "
				+ path);
		...
		SharedTaxiSolution[] sa = null;
		if (requestAirports != null && requestAirports.length > 0) {
			sa = new SharedTaxiSolution[requestAirports.length];
			DichungService dichungService = new DichungService();
			for (int i = 0; i < requestAirports.length; i++) {
				sa[i] = dichungService
						.computeSharedTaxiHanoiNoiBaiSolution(requestAirports[i]);
			}
		}

		// compute solution for long trip requests
		/*
		 * Su Phu's code SharedLongTripInput[] requestLongTrips =
		 * input.getLongtripInput(); SharedLongTripSolution[] sl = new
		 * SharedLongTripSolution[requestLongTrips.length]; for(int i = 0; i <
		 * requestLongTrips.length; i++){ SharedLongTripService
		 * dichungDuongdaiService = new
		 * SharedLongTripService(requestLongTrips[i]); sl[i] =
		 * dichungDuongdaiService.computeSharedLongTrip(); }
		 */
		// Trung's code
		SharedLongTripInput[] requestLongTrips = input.getLongtripInput();

		/*
		 * if(requestLongTrips.length == 0){ return null; }
		 */
		...
		SharedLongTripRequest[] requests = new SharedLongTripRequest[requestLst
				.size()];
		...
		int forbidenStraightDistance = requestLongTrips[0]
				.getForbidenStraightDistance();
		int forbidenTimeDistance = requestLongTrips[0]
				.getForbidenTimeDistance();
		int maxStandardSharingDistance = requestLongTrips[0]
				.getMaxStandardSharingDistance();
		int maxHighTrafficSharingDistance = requestLongTrips[0]
				.getMaxHighTrafficSharingDistance();
		int algorithmMaxTime = requestLongTrips[0].getAlgorithmMaxTime();
		double approximationDistanceFactor = requestLongTrips[0]
				.getApproximationDistanceFactor();
		...
		SharedLongTripInput sharedLongTripInput = new SharedLongTripInput(
				requests, minimumSharedPrice, vehicleCapacities, maxWaitTime,
				forbidenStraightDistance, forbidenTimeDistance,
				maxStandardSharingDistance, maxHighTrafficSharingDistance,
				algorithmMaxTime, approximationDistanceFactor, eps, stdSpeed,
				highTrafficSpeed, stableSpeed, deltaRequestTime);
		...
		// convert long trip solutions into unique format of shared a ride
		// solution
		...
		for (int i = 0; i < sl.length; i++) {
			...
			for (int j = 0; j < r.length; j++) {
				...
				tr[j] = new SharedTaxiRoute(te, r[j].getTaxiType(),
						r[j].getNbPeople(), "-");
			}
			tmp[i] = new SharedTaxiSolution(-1, -1,
					requestLongTrips[i].getRequests().length, tr);
		}

		if(sa == null){// no airport request, then return result of long-trips
			return tmp;
		}
		
		
		// concatenate result airport and long-trips
		SharedTaxiSolution[] sol = new SharedTaxiSolution[sa.length
				+ tmp.length];
	...
	public SharedTaxiSolution computeSharedTaxiSolution(
			HttpServletRequest request, @RequestBody SharedTaxiInput input) {
		String path = request.getServletContext().getRealPath(
				"ezRoutingAPIROOT");
		System.out.println(name() + "::computeSharedTaxiSolution, path = "
				+ path);
		DichungService dichungService = new DichungService();
		SharedTaxiSolution sol = dichungService
				.computeSharedTaxiHanoiNoiBaiSolution(input);

		writeRequest(input, sol);

		return sol;
		/*
		 * HashMap<SharedTaxiRequest, Integer> mRequest2ID = new
		 * HashMap<SharedTaxiRequest, Integer>(); SharedTaxiRequest[] requests =
		 * input.getRequests(); int N = requests.length; for (int i = 0; i <
		 * requests.length; i++) mRequest2ID.put(requests[i], i);
		 * 
		 * GoogleMapsQuery G = new GoogleMapsQuery();
		 * 
		 * int[][] travelTimes = new int[N][N];// travelTimes[i][j] is the
		 * travel // time from pickup point of request // i to pickup point of
		 * request j for (int i = 0; i < requests.length; i++) { for (int j = 0;
		 * j < requests.length; j++) { int t =
		 * G.getTravelTime(requests[i].getPickupAddress(),
		 ...
		 * requests[j].getPickupAddress()); System.exit(-1); } } }
		 * 
		 * 
