...
public class TSP {
	...
	public ArrayList<Point> lsInitTSP(){
		...
		System.out.println(clientPoints);
		...
		IFunctionVR cost = new AccumulatedEdgeWeightsOnPathVR(awe, XR.endPoint(0));
		...
		gs.search(300, 10);
		localsearch.domainspecific.vehiclerouting.vrp.entities.Point p=XR.getStartingPointOfRoute(0);
		ArrayList<Point> lSol= new ArrayList<Point>();
		lSol.add(mapPP.get(p));
		while(true){
			p=XR.next(p);
			lSol.add(mapPP.get(p));
			if(p==XR.getTerminatingPointOfRoute(0)) break;
		}
		return lSol;
	}
	String name(){
