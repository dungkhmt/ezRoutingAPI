...
public class GreedyConstructiveOrderLoadConstraint {
	...
	private int[][][] occ;// occ[iw][il][ih] = 1 if cell occ[iw][il[ih] is occupied
	...
	private int[][][] bku_occ;// occ[iw][il][ih] = 1 if cell occ[iw][il[ih] is occupied
	...
	public GreedyConstructiveOrderLoadConstraint(){
		
	}
	public GreedyConstructiveOrderLoadConstraint(Model3D model){
		this.model = model;
	...
	public void backup(){
		...
		for(int i = 0; i < markW.length; i++) bku_markW[i] = markW[i];
		for(int i = 0; i < markL.length; i++) bku_markL[i] = markL[i];
		for(int i = 0; i < markH.length; i++) bku_markH[i] = markH[i];
	...
	public void restore(){
		...
		for(int i = 0; i < bku_markW.length; i++) markW[i] = bku_markW[i];
		for(int i = 0; i < bku_markL.length; i++) markL[i] = bku_markL[i];
		for(int i = 0; i < bku_markH.length; i++) markH[i] = bku_markH[i];
	...
	public void init(){
		...
		bku_occ = new int[model.getContainer().getWidth()][model.getContainer().getLength()][model.getContainer().getHeight()];
			...
			for(Position3D p: candidate_positions){
				...
				for(Item3D I: gen_items){
					...
					if(feasiblePosition(p.getX_w(), p.getX_l(), p.getX_h(), I.getWidth(), I.getLength(), I.getHeight())){
						moves.add(new Move3D(p,I.getWidth(),I.getLength(),I.getHeight(),item.getItemID()));
					}
				}				
			}
		...
		for(int i = 0; i < solution.size(); i++){
			...
			if(m.getPosition().getX_w() > 0){
				for(int j = 0; j < i; j++){
					...
					if(mj.getPosition().getX_w() + mj.getW() == m.getPosition().getX_w()){
			...
			if(m.getPosition().getX_l() > 0){
				for(int j = 0; j < i; j++){
					Move3D mj = solution.get(j);
					if(mj.getPosition().getX_l() + mj.getL() == m.getPosition().getX_l()){
			...
			if(m.getPosition().getX_h() > 0){
				for(int j = 0; j < i; j++){
					Move3D mj = solution.get(j);
					if(mj.getPosition().getX_h() + mj.getH() == m.getPosition().getX_h()){
						sh = "Tren item " + mj.getItemID();
					}
				}
			}
			System.out.println("solution item " + m.getItemID() + ": " + m.getW() + "," + m.getL() + "," + m.getH() + " at " + m.getPosition()+
					" : " + sw + ", " + sl + ", " + sh);
		}
	}
	public void solve(){
		...
		for(int i = 0;i < o.length; i++){
			...
			for(Position3D p: candidate_positions){
				...
				for(Item3D I: gen_items){
					//System.out.println("Consider item " + item + " with permutation " + I);
					if(feasiblePosition(p.getX_w(), p.getX_l(), p.getX_h(), I.getWidth(), I.getLength(), I.getHeight())){
						moves.add(new Move3D(p,I.getWidth(),I.getLength(),I.getHeight(),item));
					}
				}				
			}
	...
	public boolean overlap(int pos_w1, int w1, int pos_l1, int l1, int pos_w2, int w2, int pos_l2, int l2){
		return pos_w1 + w1 <= pos_w2 || pos_w2 + l2 <= pos_w1 ||
	...
	public boolean feasiblePosition(int pos_w, int pos_l, int pos_h, int w, int l, int h){
		...
		for(int i = 0; i < solution.size(); i++){
			...
			if(volumn(m.getW(),m.getL(),m.getH()) < volumn(w,l,h) && 
					overlap(m.getPosition().getX_w(),m.getW(),m.getPosition().getX_l(),m.getL(), pos_w, w, pos_l, l) && h > m.getH()){
				return false;
			}
		}
		return true;
